...
ISOdothexToISOdecimal <- function(.data){
  ...
  for(i in .data){
    if(ISO11784Tools::get_iso11784_format(i) != 'ISOdothex'){
      ...
      # calculations LEFT
      manufacturer <- ISO11784Tools::hex2dec(manufacturer)
      # calculations RIGHT
      animalID <- ISO11784Tools::hex2dec(animalID)
...
ISO64bitLeftToISODecimal <- function(.data){
  ...
  for(i in .data){
    if(ISO11784Tools::get_iso11784_format(i) != 'ISO64bitl'){
      ...
      manufacturer <- bin2dec(stringr::str_sub(ISO11784Tools::hex2bin(stringr::str_sub(i,5,7)),1,10))
      animalID <- stringr::str_pad(ISO11784Tools::bin2dec(stringr::str_sub(ISO11784Tools::hex2bin(stringr::str_sub(i,7,16)),3,40)),
        width = 12,
        pad = "0",
        side = 'left')
      out <- append(out,paste0(manufacturer,animalID))
...
ISO64bitRightToISODecimal <- function(.data){
  ...
  for(i in .data){
    if(ISO11784Tools::get_iso11784_format(i) != 'ISO64bitr'){
      ...
      manufacturer <- ISO11784Tools::bin2dec(stringi::stri_reverse(stringr::str_sub(ISO11784Tools::hex2bin(stringr::str_sub(i,10,12)),3,12)))
      animalID <- stringr::str_pad(ISO11784Tools::bin2dec(stringi::stri_reverse(stringr::str_sub(ISO11784Tools::hex2bin(stringr::str_sub(i,1,10)),1,38))),
        width = 12,
        pad = "0",
        side = 'left')
      out <- append(out,paste0(manufacturer,animalID))
